export class Drone {
    ...
    carregarEntrega(peso, destinoX, destinoY) {
        if (peso > this.pesoMaximo)
            throw new Error("Peso excede capacidade do drone");
    ...
    atualizarPosicao(passo = 2) {
        if (this.destinoX === undefined || this.destinoY === undefined)
            return;
        ...
        if (Math.abs(dx) <= passo && Math.abs(dy) <= passo) {
            ...
            if (this.estado === 'emRota')
                this.estado = 'entregando';
            else if (this.estado === 'retornando')
                this.estado = 'idle';
        }
        else {
