...
export class Simulacao {
    constructor(baseX, baseY, canvasWidth, canvasHeight) {
        ...
        for (let r = 0; r < this.ROWS; r++) {
            for (let c = 0; c < this.COLS; c++) {
                ...
                if (Math.abs(r - baseGY) <= 2 && Math.abs(c - baseGX) <= 3)
                    continue;
    ...
    adicionarEntrega(desc, peso, prioridade = 2) {
        ...
        const entrega = new Entrega(this.entregas.length + 1, desc, peso, destino.x, destino.y, prioridade);
    ...
    iniciarDronesEntregas() {
        if (!this.drones.some(d => d.nome === "Drone01"))
            this.adicionarDrone("Drone01", 5);
        if (!this.drones.some(d => d.nome === "Drone02"))
            this.adicionarDrone("Drone02", 5);
        if (!this.entregas.some(e => e.descricao === "Entrega 1"))
            this.adicionarEntrega("Entrega 1", 1);
        if (!this.entregas.some(e => e.descricao === "Entrega 2"))
            this.adicionarEntrega("Entrega 2", 1);
        if (!this.entregas.some(e => e.descricao === "Entrega 3"))
            this.adicionarEntrega("Entrega 3", 1);
    }
    get estaAtiva() {
