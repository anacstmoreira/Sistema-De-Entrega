...
function alocarEntrega(drone: Drone) {

    if (drone.entregaAtual) return;


    const entrega = sim.getEstado().entregas.find(e =>
        e.status === 'pendente' &&
        !e.drone &&
...
if (sim.getEstado().drones.length === 0) sim.adicionarDrone("Drone01", 5, BASE_X, BASE_Y);
if (sim.getEstado().drones.length === 1) sim.adicionarDrone("Drone02", 5, BASE_X, BASE_Y);
if (sim.getEstado().entregas.length === 0) sim.adicionarEntrega("Entrega 1", 1, Math.random() * canvas.width, Math.random() * canvas.height);
if (sim.getEstado().entregas.length === 1) sim.adicionarEntrega("Entrega 2", 1, Math.random() * canvas.width, Math.random() * canvas.height);
if (sim.getEstado().entregas.length === 2) sim.adicionarEntrega("Entrega 3", 1, Math.random() * canvas.width, Math.random() * canvas.height);
...
//Botões
const modalDrone = document.getElementById('modalDrone') as HTMLDivElement;
const modalEntrega = document.getElementById('modalEntrega') as HTMLDivElement;
...
document.getElementById('confirmDrone')?.addEventListener('click', () => {
    const nome = (document.getElementById('droneNome') as HTMLInputElement).value || `Drone`;
    const peso = parseFloat((document.getElementById('dronePeso') as HTMLInputElement).value || "5");
...
document.getElementById('confirmEntrega')?.addEventListener('click', () => {
    const descricao = (document.getElementById('entregaDesc') as HTMLInputElement).value || "Entrega";
    const peso = parseFloat((document.getElementById('entregaPeso') as HTMLInputElement).value || "1");
...
// Seleciona elementos
const modalBemVindo = document.getElementById('modalBemVindo') as HTMLDivElement;
const btnFecharBemVindo = document.getElementById('btnFecharBemVindo') as HTMLButtonElement;
